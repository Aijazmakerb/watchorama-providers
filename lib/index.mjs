...
const getSimilarityBetweenStrings = (firstParam, secondParam) => {
...
async function getFlixhqId(ctx, media) {
  ...
  const searchResults = await ctx.proxiedFetcher(`/search/${mediaTitle.replace(/[\W_]+/g, "-")}`, {
    baseUrl: flixHqBase
  });
  const doc = load(searchResults);
  let results = doc(".film_list-wrap > div.flw-item").toArray().map((el) => {
    var _a2;
    const query = doc(el);
    const id = (_a2 = query.find("div.film-poster > a").attr("href")) == null ? void 0 : _a2.slice(1);
    ...
    const type = query.find("div.film-detail > div.fd-infor > span.float-right").text() === "Movie" ? "movie" : "show";
    if (!id || !title || !year || !type)
      return null;
    return {
      id,
      title,
  ...
  results.sort((a, b) => {
    ...
    const firstTitle = (a == null ? void 0 : a.title) === "string" ? a.title : "";
    const secondTitle = (b == null ? void 0 : b.title) === "string" ? b.title : "";
  ...
  results = results.filter((result) => {
    if (media.type === "movie")
      return (result == null ? void 0 : result.type) === "movie";
    if (media.type === "show")
      return (result == null ? void 0 : result.type) === "show";
    return false;
  });
  return ((_a = results[0]) == null ? void 0 : _a.id) ?? null;
}
